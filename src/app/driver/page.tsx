    ...
    if (isActive) {
      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        ...
        (error) => {
          ...
          alert('Ошибка доступа к GPS. Убедитесь, что геопозиция включена в настройках браузера.');
      ...
      <div className="p-4 border-b border-[var(--border-color)] bg-[#12121a] flex items-center justify-between z-10">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-extrabold uppercase tracking-widest text-[var(--text-secondary)]">
            {isActive ? 'Смена Активна' : 'Смена Закрыта'}
          </span>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-3 mt-1">
              ...
              >
                {MOCK_ROUTES.map((r) => (
                  <option key={r.route_id} value={r.route_id} className="bg-[#12121a] text-white">
                    Маршрутка {r.route_short_name} ({r.route_long_name})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="px-3 py-2 text-xs font-bold rounded-lg bg-[rgba(255,255,255,0.03)] border border-[var(--border-color)] text-white outline-none focus:border-[var(--accent)] cursor-pointer disabled:opacity-50"
              >
                <option value="v1" className="bg-[#12121a] text-white">Машина 1 (v1)</option>
                <option value="v2" className="bg-[#12121a] text-white">Машина 2 (v2)</option>
                <option value="v3" className="bg-[#12121a] text-white">Машина 3 (v3)</option>
                <option value="v4" className="bg-[#12121a] text-white">Машина 4 (v4)</option>
              </select>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="glass-panel p-4 flex flex-col items-center gap-1 text-center">
              ...
              <span className="text-2xl font-black text-white">
                {speed} <span className="text-xs font-bold text-[var(--text-secondary)]">км/ч</span>
              </span>
            </div>
            ...
            <div className="glass-panel p-4 flex flex-col items-center gap-1 text-center">
              ...
              <span className="text-2xl font-black text-white">
                {bearing}° <span className="text-xs font-bold text-[var(--text-secondary)]">напр.</span>
              </span>
            </div>
          </div>
        )}
        ...
        >
          {isActive ? (
            <>
              ...
              Завершить смену (Стоп GPS)
            </>
          ) : (
            <>
              <Play className="w-4 h-4 fill-current" />
              Начать смену (Старт GPS)
            </>
          )}
        </button>
      </div>
